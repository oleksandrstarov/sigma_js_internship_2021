import { API_SEARCH_URL, API_KEY, API_POPULAR_URL, API_URL, API_IMG_URL } from '../constants/api';
    ...
    this.getDataByIds = (arr: number[]) => {
        let funk = () => {
            let urls = arr.map((id: number) => `${API_URL}/${id}?${API_KEY}`),
                requests = urls.map(async (url: any) => await axios.get(url).then(res => res.data))
